...
--// GLOW FINO (SEM BORDA VISÍVEL) \\--
function Library:AddOutline(Frame: GuiObject)
    -- Ignorar grupos (Groupbox / containers grandes)
    if Frame:IsA("Frame") and Frame.Name:lower():find("group") then
        return nil, nil
    end

    -- Glow bem suave (quase invisível, só brilho)
    local Glow = New("UIStroke", {
        Color = "AccentColor",
        Thickness = 1.2, -- fino
        Transparency = 0.85, -- quase invisível (sem linha dura)
        ApplyStrokeMode = Enum.ApplyStrokeMode.Border,
        LineJoinMode = Enum.LineJoinMode.Round,
        Parent = Frame,
    })

    -- Camada extra MUITO leve (efeito glow mesmo)
    local SoftGlow = New("UIStroke", {
        Color = "AccentColor",
        Thickness = 2,
        Transparency = 0.92,
        ApplyStrokeMode = Enum.ApplyStrokeMode.Border,
    ...
    return Glow, SoftGlow
end

--// Deprecated \\--
function Library:MakeOutline(Frame: GuiObject, Corner: number?, ZIndex: number?)
	warn("Obsidian:MakeOutline deprecated (agora usa glow leve).")

    -- Ignorar groups também aqui
    if Frame:IsA("Frame") and Frame.Name:lower():find("group") then
        return nil, nil
    end
