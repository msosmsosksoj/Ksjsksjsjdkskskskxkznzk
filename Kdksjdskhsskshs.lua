...
GroupboxHolder.InputBegan:Connect(function(input)
    if input.UserInputType == Enum.UserInputType.MouseButton1 then
        ...
        GroupboxContainer.Visible = Open

        if Open then
            GroupboxHolder.Size = UDim2.new(
                1, 0,
                0,
                (GroupboxList.AbsoluteContentSize.Y / Library.DPIScale) + 49 + ExtraPadding
            )
        else
            GroupboxHolder.Size = UDim2.new(1, 0, 0, 34)
        end

        Arrow:TweenRotation(Open and 0 or -90, Enum.EasingDirection.Out, Enum.EasingStyle.Quad, 0.2, true)
    end
end)

function Groupbox:Resize()
    if GroupboxContainer.Visible then
        GroupboxHolder.Size = UDim2.new(1, 0, 0, (GroupboxList.AbsoluteContentSize.Y / Library.DPIScale) + 49)
    end
end
end)
...
if Library.IsMobile then
    ...
    local Toggleimg = New("ImageButton", {
        ...
        BackgroundColor3 = Color3.fromRGB(40, 40, 40), -- Cor escura do fundo
        Position = UDim2.fromOffset(6, 6),
        ZIndex = 10,
        Parent = ScreenGui,
        DPIExclude = {
    ...
    -- Conecta o clique para alternar a GUI
    Toggleimg.MouseButton1Click:Connect(function()
        Library:Toggle()
    end)

    -- Aplica o corner radius
    local UICorner = New("UICorner", {
        CornerRadius = UDim.new(0, WindowInfo.CornerRadius),
        Parent = Toggleimg,
    })

    -- Cria o outline preto
    local Outline = New("Frame", {
        BackgroundColor3 = Color3.fromRGB(0, 0, 0), -- Preto
        BorderSizePixel = 0,
        Position = UDim2.fromOffset(-2, -2),
        Size = UDim2.new(1, 4, 1, 4),
        ZIndex = 9,
    ...
    New("UICorner", {CornerRadius = UDim.new(0, WindowInfo.CornerRadius), Parent = Outline})

    -- Permite arrastar o botão
    Library:MakeDraggable(Toggleimg, Toggleimg, true)

    -- Ajusta para o lado direito se configurado
