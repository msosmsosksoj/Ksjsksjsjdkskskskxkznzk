...
function Library:AddOutline(Frame: GuiObject)
    -- ❌ IGNORAR GROUPBOX / GROUPS
    local name = Frame.Name:lower()
    if name:find("groupbox") or name:find("group") then
        return nil, nil
    end

    -- 🎨 Cor igual da imagem (laranja)
    local GlowColor = Color3.fromRGB(255, 140, 45)

    -- Glow principal (invisível inicialmente)
    ...
    -- 🧠 HOVER EFFECT (igual da imagem)
    Frame.MouseEnter:Connect(function()
        Glow.Transparency = 0.85
        SoftGlow.Transparency = 0.93
    end)

    Frame.MouseLeave:Connect(function()
        Glow.Transparency = 1
        SoftGlow.Transparency = 1
    end)
...
function Library:MakeOutline(Frame: GuiObject, Corner: number?, ZIndex: number?)
	warn("Obsidian:MakeOutline deprecated (usando glow).")
